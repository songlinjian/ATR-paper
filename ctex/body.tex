...
\cite{a-question-of-dns-protocols} shows that about 17\% of resolvers in the samples can
...
\subsection{Background}
   ...
   Even though IPv6 fragmentation are observed by some studies with
   notable packet drop rate but it is not a well know issue at least for
   end users. We will introduce a brief background how DNS evolved into
   this situation in this section.

   DNS has an inherent mechanism defined in  RFC1035 \cite{RFC1035}  to handle large
   ...
   response and TCP fall back firstly in 1999 and obsoleted by RFC6891 \cite{RFC6891}
   ...
   measurement study \cite{a-question-of-dns-protocols} showed that about 17\% of resolvers
...
inefficiency \cite{kent1987fragmentation}, security vulnerability
\cite{herzberg2013fragmentation} and
unreliability at high data rates [RFC4963]. In addition, it was
reported \cite{RFC7872, de2012discovering} that some networks deliberately
filter fragments especially in IPv6 which makes IP fragments
less reliable.

F. Gont et al.\cite{RFC7872} adopt Active measurements by sending
...
et al. \cite{de2012discovering} sent fragmented packets to IPv6 probes.
They found that during the test, 10\% percent of the probes
did not receive fragmented packets. Different from RFC7872 \cite{RFC7872},
...
drop rate which is similar with F. Gont's work \cite{RFC7872}.

Song et al.\cite{RFC8483} set up a IPv6 only DNS testbed and perform the
...
However, similar with \cite{de2012discovering}, they only tested hundreds of probes and paths
...
during KSK rollover \cite{ksk-rollover-icann-v2labs.ripe.net}.

In 2015 Zhu et al. \cite{zhu2015connection} proposed a Connection-Oriented DNS called T-DNS
...
To address the IP fragmentation issue, Sivaraman et al.\cite{draft-muks-dns-message-fragments-00.txt}
...
AS31034 November & 0.47\% & 267,478 & ARUBA-ASN & IT \\
